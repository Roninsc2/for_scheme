...
public:
    ...
    virtual void UpdateStack(std::vector<std::shared_ptr<ExprType>>& allocator);
private:
    int value;
};
...
public:
    ...
    virtual void UpdateStack(std::vector<std::shared_ptr<ExprType>>& stack, std::vector<std::shared_ptr<ExprType>> allocator);
private:
    size_t valueNumber;
};
